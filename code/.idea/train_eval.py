...
def train(config,model,a,train_data1,dev_data1,train_data2,dev_data2,train_data3,dev_data3):
    ...
    group1 = ['lstm.', 'classifier_3.', 'classifier_2.', 'classifier_1.']
    ...
    """
      optimizer_grouped_parameters = [
        {'params': [p for n, p in param_optimizer if
         ...
         'weight_decay_rate': 0.01, 'lr': config.learning_rate * 100},
        {'params': [p for n, p in param_optimizer if
                    not any(nd in n for nd in no_decay) and any(nd in n for nd in group_all)],
         'weight_decay_rate': 0.01, 'lr': config.learning_rate},
        {'params': [p for n, p in param_optimizer if
                    any(nd in n for nd in no_decay) and not any(nd in n for nd in group_all)],
         'weight_decay_rate': 0.00, 'lr': config.learning_rate * 100},
        {'params': [p for n, p in param_optimizer if
                    any(nd in n for nd in no_decay) and any(nd in n for nd in group_all)], 'weight_decay_rate': 0.00,
         'lr': config.learning_rate},

    ]
    """

    optimizer=BertAdam(optimizer_grouped_parameters,lr=config.learning_rate,warmup=0.05,t_total=len(a)*config.num_epochs) #total number of training steps for the learning
...
def predict(config,model,data_iter,file,id):
    ...
    if id==1:
        ...
        dic[0]='100';dic[1]='101';dic[2]='102';dic[3]='103';dic[4]='104';dic[5]='106';dic[6]='107';dic[7]='108';dic[8]='109';dic[9]='110';dic[10]='112';dic[11]='113';dic[12]='114';dic[13]='115';dic[14]='116'
    else:
        dic[0]='like';dic[1]='happiness';dic[2]='sadness';dic[3]='fear';dic[4]='anger';dic[5]='disgust';dic[6]='surprise'
    with torch.no_grad():
